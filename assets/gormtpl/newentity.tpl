...
type {{.Table}}Dao interface {
	Add(model *{{.Table}}) (err error)
	Save(model *{{.Table}}) (err error)
	Delete(query interface{}, args ...interface{}) (err error)
	First(query interface{}, args ...interface{}) (model {{.Table}}, err error)
	Find(query interface{}, page *Pagination, args ...interface{}) (models []{{.Table}}, err error)
	Count(where interface{}, args ...interface{}) (count int64, err error)
}

type {{.Table}}DaoImpl struct {
	db *gorm.DB
}

// 添加记录
func (r *{{.Table}}DaoImpl) Add(model *{{.Table}}) (err error) {
	err = r.Db.Create(model).Error
	return
}

// 更新保存记录
func (r *{{.Table}}DaoImpl) Save(model *{{.Table}}) (err error) {
	err = r.Db.Save(model).Error
	return
}

// 软删除：结构体需要继承Base model 有delete_at字段
func (r *{{.Table}}DaoImpl) Delete(query interface{}, args ...interface{}) (err error) {
	//return r.Db.Unscoped().Where(query, args...).Delete(&{{.Table}}{}).Error //硬删除
	return r.Db.Where(query, args...).Delete(&{{.Table}}{}).Error
}

// 根据条件获取单挑记录
func (r *{{.Table}}DaoImpl) First(query interface{}, args ...interface{}) (model {{.Table}}, err error) {
...
func (r *{{.Table}}DaoImpl) Find(query interface{}, page *Pagination, args ...interface{}) (models []{{.Table}}, err error) {
	if page == nil {
...
// 获取总记录条数
func (r *{{.Table}}DaoImpl) Count(where interface{}, args ...interface{}) (count int64, err error) {
